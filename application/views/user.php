        ...
        <tbody>
          ...
          foreach ($user as $ssw): ?>
            <tr class="text-center filter-row" data-type="<?= htmlspecialchars($ssw->type ?? '', ENT_QUOTES, 'UTF-8') ?>">
              ...
              <td>Rp. <?= number_format((float) str_replace('.', '', $ssw->tarif ?? 0), 0, ',', '.') ?></td>
          ...
          <div class="modal-body">
            ...
            <div class="row">
              ...
              <div class="col-md-4">
                <div class="form-group">
                  <label>Site</label>
                  <input type="text" class="form-control" value="<?= htmlspecialchars($ssw->site ?? '', ENT_QUOTES, 'UTF-8') ?>" readonly>
                  ...
                  <select name="bandwith_id" id="edit_bandwith_id" class="form-control">
                    <?php foreach ($bandwith as $bw): ?>
                      <option value="<?= $bw->id_bw ?? '' ?>" data-harga="<?= $bw->harga ?? '' ?>">
                        <?= htmlspecialchars($bw->namapaket ?? '', ENT_QUOTES, 'UTF-8') ?>
                      </option>
                    <?php endforeach; ?>
                  </select>
                </div>
                <div class="form-group">
                  ...
                  <select name="sesi_id" id="edit_sesi_id" class="form-control">
                    <?php foreach ($sesi as $si): ?>
                      <option value="<?= $si->id_sesi ?? '' ?>"><?= htmlspecialchars($si->namasesi ?? '', ENT_QUOTES, 'UTF-8') ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
              </div>
